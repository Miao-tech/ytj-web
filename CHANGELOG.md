...
| `src/components/sensornew.js` | 添加蜂鸣器，5列布局，删除手势传感器 | 传感器监控UI |
| `src/components/multimeter.js` | 简化为3档，3列布局 | 万用表控制UI |
| `src/store_integrated_machine_slice.js` | 添加led10/led11/buzzer状态和action | Redux全局状态 |
| `src/request/io.js` | 更新协议解析：0x05/0x06/0x0A/0x0D/0x19/0x1A | WebSocket数据处理 |
| `src/request/api.js` | 删除ACV/DCA相关API调用 | API接口 |
...
这3个红外传感器是一体的，工作原理如下：如果我发送 0x0b 0x00 0x01 0xfe请求3个红外传感器的状态，返回的结果解释如下：

0x0b 0x00 0x01 0xfe : 1号红外传感器有遮挡
0x0b 0x00 0x02 0xfe : 2号红外传感器有遮挡
0x0b 0x00 0x04 0xfe  :3号红外传感器有遮挡
0x0b 0x00 0x03 0xfe  :1号和2号红外传感器有遮挡

0x0b 0x00 0x05 0xfe  :1号和3号红外传感器有遮挡

0x0b 0x00 0x06 0xfe  :2号和3号红外传感器有遮挡

0x0b 0x00 0x07 0xfe  :1号和2号和3号红外传感器有遮挡

因此可能只需要一个更新（获取状态）按钮，且不需要启动停止相关API，传感器模块可以只保持5列布局，这三个传感器可另起一个新行。你明白我的意思吗，请参考我的意见作出修改
